...
for name in "${certs[@]}";do
  ...
  keytool -genkeypair -keyalg RSA -alias mykey -keystore /certs/${name}.jks -storepass password -validity 365 -dname "CN=${name}, OU=MyOrganization, O=MyCompany, L=MyCity, ST=MyState, C=MyCountry" -storetype JKS -noprompt
done
